...
/// check if the line starts with a number (for skipping section headers)
...
pub fn parse_instance(path: &str) -> Result<Instance> {
    ...
    let next = |i: &mut usize| {
        let v = lines.get(*i).cloned().unwrap_or_default();
        *i += 1;
        v
    };
    ...
    // 1) Map (width height)
    // =========================
    // skip headers until we see a line that starts with a number
    while i < lines.len() && !peek_is_number(&lines[i]) {
        i += 1;
    }
    ...
    // helper: get next numeric line (skip 'crane section', 'storage section', etc.)
    ...
    let n_cranes: i32 = n_cranes_line
        .split_whitespace()
        .next()
        .unwrap_or("0")
        .parse()
    ...
    for _ in 0..n_cranes {
        ...
        // each dispatch = (id, x, y) -> we keep only (x, y)
        // nums: 0   1  2  3  4  5  6  7  8
        //       id x1 y1 x2 y2 nd did x  y
        for k in 0..nd {
            let base = 6 + 3 * k;
            // let disp_id = nums[base];     // we ignore it for now
            let x = nums[base + 1];
            let y = nums[base + 2];
            dispatches.push((x, y));
        }
    ...
    // 6) Demands
    // =========================
    // After this point, the toy file starts the "demand section" with a bunch of
    // extra words ("demand section", "demand crane 0", "ship 0", etc.)
    // Your original parser just read line by line and matched "load"/"unload".
    // We keep that — it's robust enough.
    ...
    while i < lines.len() {
        ...
        match op.as_str() {
            ...
            _ => {
                // ignore "demand section", "ship 0", "1 0", etc.
            }
        }
    }
